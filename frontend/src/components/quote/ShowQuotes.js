...
const ShowQuotes = () => {
    ...
    return (
        <div className="bg-white shadow mt-20 md:w-2/3 lg:w-1/2 xl:w-1/3 mx-auto max-h-[400px] overflow-y-auto ">
            <h2 className="text-xl font-semibold text-gray-900 px-4 py-2 border-b border-gray-200 text-center bg-gray-100 border ">Active quotes</h2>
            <ul>
                {quotes.map((quote, index) => (
                    <li key={index} className={index > 0 ? "cursor-pointer hover:bg-gray-50 border-t border-gray-200" : "cursor-pointer hover:bg-gray-50"}>
                        <div className="px-2 py-3 sm:px-6">
                            <div className="flex items-center justify-between">
                                <h3 className="sm:text-lg leading-6 font-medium text-gray-900 hover:underline">{quote.title}</h3>
                                <p className="mt-1 max-w-xs sm:max-w-2xl text-xs sm:text-xxs text-gray-500 ">{dateConverter(quote.date)}</p>
                            </div>
                            <div className="mt-2 flex items-center justify-between">
                                ...
                                <button onClick={() => removeQuote(quote)}>
                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 24 24" className='hover:fill-red-500'>
